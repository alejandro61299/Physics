...
bool ModulePhysics3D::CleanUp()
{
	...
	for (p2List_item<btCollisionShape*> *item = colliders.getFirst(); item ; item = item->next) 
	{
		if (item->data)
		{
			delete item->data;
	...
	for (p2List_item<PhysBody3D*> *item = pbodies.getFirst(); item; item = item->next)
	{
		if (item->data)
		{
			delete item->data;
